...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($senha) && !validar_senha($senha)) {
        ...
        header("location: /cadastro/usuario/alterar.php?id=" . $id);
        exit;
    }
    ...
    // Se senha foi preenchida, atualiza com nova senha (hash)
    if (!empty($senha)) {
        $senha_hash = hash_senha($senha);
        $sql = "UPDATE usuario SET nome = ?, usuario = ?, senha = ? WHERE id = ?";
        $update = $conn->prepare($sql);
        $update->bind_param("sssi", $nome, $usuario, $senha_hash, $id);
    } else {
...
<div class="w3-container">
    ...
    <form action='alterar.php' method='post' class="w3-container w3-card-2 w3-margin-top">
        <input type="hidden" name="id" value="<?php echo escape($usuario_data['id']); ?>">

        <div class="w3-row-padding w3-margin-top">
            <div class="w3-col s4">
                <label for="nome">Nome Completo</label>
                <input class="w3-input w3-border" type="text" id="nome" name="nome" value="<?php echo escape($usuario_data['nome']); ?>" required>
            </div>
            <div class="w3-col s4">
                <label for="usuario">Nome de Usuário</label>
                <input class="w3-input w3-border" type="text" id="usuario" name="usuario" value="<?php echo escape($usuario_data['usuario']); ?>" required>
            </div>
            <div class="w3-col s4">
                <label for="senha">Nova Senha (deixe em branco para manter)</label>
                <input class="w3-input w3-border" type="password" id="senha" name="senha" minlength="6">
            </div>
        </div>
